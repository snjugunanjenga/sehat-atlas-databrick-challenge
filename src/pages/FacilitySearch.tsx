...
const SUGGESTIONS = [
  "Find the nearest facility in rural Bihar that can perform an emergency appendectomy and uses part-time doctors",
  "Dialysis centres in Tamil Nadu with 24/7 operation",
  "Trauma care in rural Jharkhand with high trust score",
  "Oncology facilities in Maharashtra",
];
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Ask in plain English. The agent retrieves, scores, validates, and cites every claim."
      />
            ...
            >
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  placeholder="e.g. nearest rural Bihar facility for emergency appendectomy with part-time doctors"
                  className="h-11 pl-9"
                />
              </div>
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-0.5 flex items-center gap-1 text-xs text-muted-foreground">
                            <MapPin className="h-3 w-3" /> {facility.district}, {facility.state} · PIN {facility.pin} · {facility.beds} beds · {facility.rural ? "Rural" : "Urban"}
                          </p>
                        </div>
                      ...
                      <CollapsibleContent className="space-y-3 border-t pt-3">
                        <div>
                          ...
                          <div className="mt-2 space-y-2">
                            {facility.citations.length === 0 && (
                              <p className="text-xs italic text-muted-foreground">No structured citations extracted yet.</p>
                            )}
                            {facility.citations.map((c, i) => (
                        ...
                        {facility.contradictions.length > 0 && (
                          <div>
                            ...
                            <ul className="mt-2 space-y-1 text-xs">
                              {facility.contradictions.map((c, i) => (
        ...
        {!result && (
          <Card>
            <CardContent className="p-12 text-center text-sm text-muted-foreground">
              Ask anything about the 240 indexed facilities — capability, location, staffing, or trust.
            </CardContent>
          </Card>
        )}
      </div>
